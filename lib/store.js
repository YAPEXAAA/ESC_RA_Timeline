...
// Drafts live in their own blob, always overwritten in place — only ever
// holds the most recently uploaded draft (one week at a time).
const DRAFT_BLOB_PATH = 'schedules.draft.json';
...
// Draft state is just the parsed workbook plus minimal meta — always fully
// replaced, never merged with a previous draft.
async function loadDraft() {
  try {
    const result = await get(DRAFT_BLOB_PATH, { access: 'private', useCache: false });
    if (!result) return null;
    const text = await new Response(result.stream).text();
    return JSON.parse(text);
  } catch (err) {
    console.error('Failed to read draft blob, treating as empty:', err.message);
    return null;
  }
}

async function saveDraftUpload({ employees, scheduleByDate, datesFound, sheetsParsed }, filename) {
  const state = {
    employees,
  ...
  await put(DRAFT_BLOB_PATH, JSON.stringify(state), {
    access: 'private',
    contentType: 'application/json',
    addRandomSuffix: false,
    allowOverwrite: true, // draft blob always holds just the latest upload
  });
  return state;
}

module.exports = { load, save, mergeUpload, loadDraft, saveDraftUpload };